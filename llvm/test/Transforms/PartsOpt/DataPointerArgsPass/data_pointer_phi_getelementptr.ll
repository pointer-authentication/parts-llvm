...
; CHECK: %1 = call %pre.struct.i32** {{.*}}parts.data.pointer.argument{{.*}}
; CHECK: %2 = call %pre.struct.i32* {{.*}}parts.data.pointer.argument{{.*}}
...
; CHECK:  %4 = call i32* {{.*}}parts.data.pointer.argument{{.*}}
  br label %return
if.end:
  %gep2 = getelementptr inbounds %pre.struct.i32, %pre.struct.i32* %load2, i64 0, i32 2
; CHECK:  %5 = call i32* {{.*}}@llvm.parts.data.pointer.argument{{.*}}
  br label %return
return:
  %retptr = phi i32* [ %gep1, %if.then ], [ %gep2, %if.end ]
; CHECK:  %6 = call i32* {{.*}}@llvm.parts.data.pointer.argument{{.*}}
