; Check that we insert the needed functionality to authenticate a global struct with data pointers
...
; CHECK: entry:
; CHECK:   %0 = load %struct.data*, %struct.data** getelementptr inbounds (%struct.data, %struct.data* @global_struct_data, i32 0, i32 0)
; CHECK:   %1 = call %struct.data* @llvm.pa.pacda.p0s_struct.datas(%struct.data* %0, i64 {{-?[0-9]+}})
; CHECK:   store %struct.data* %1, %struct.data** getelementptr inbounds (%struct.data, %struct.data* @global_struct_data, i32 0, i32 0)
; CHECK:   ret void
; CHECK: }
