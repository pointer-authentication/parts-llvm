...
void PartsFastISel::addMetadataToCall(MachineInstrBuilder &MIB, FastISel::CallLoweringInfo &CLI, unsigned reg)
{
  ...
  if (reg) {
    ...
    partsType = PartsTypeMetadata(*Callee->getType());
  } else {
    DEBUG_PA(log->debug(FuncInfo.Fn->getName()) << "\t\t\t*** setting ignore metadata to emitted branch instruction\n");
    partsType = PartsTypeMetadataIgnored;
  }
  if (partsType == nullptr) {
    llvm_unreachable("Type Metadata results in Uninitialized value");
  }
  //assert(partsType != nullptr && "Type Metadata results in Uninitialized value");
  MIB.addMetadata(partsType.toMDNode(C));
}
*/